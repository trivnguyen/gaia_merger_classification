...
print("Load star distance")
disttrue = np.load('../data/sliced_distance.npy')
...
with open('../data/MeansAndVariance.txt') as f:
    for line in f:
        ...
        if line[0] == 'distance':
            distance_mean = float(line[1])
            distance_std = np.sqrt(float(line[2]))
            print('distance: {0}, {1}'.format(distance_mean, distance_std))
            normstatistics[2] = [distance_mean, distance_std]
        if line[0] == 'pmra_true':
...
print(normstatistics)

star_weights = {0: 1.00012,  # 598334698.0 / (598334698.0-700280),
                1: 854.422 / 5  # 598334698.0 / 700280
                }
print("Building training data generator")
# list_IDs=StarIndices[:568417963],
# list_IDs=StarIndices[568417963:568417963 + 14958367],
TrainingData = DataGenerator(list_IDs=StarIndices[:-2000000],
                             data=[ltrue, btrue, disttrue,
                                   pmratrue, pmdectrue],
...
EPCOCHS = 400
LR = 0.001
LRDECAY = 0.25 * LR / EPCOCHS / len(TrainingData)

filepath = "saved_models/UsingDistanceTrueNoRegLRPlateau_LargeBatch_ES_div5-{epoch:02d}-{val_loss:.2f}.hdf5"
...
model.save('saved_models/UsingDistanceTrue_NoRegLRPlateau_LargeBatch_ES_div5.h5')

with open('History_UsingDistanceTrue_NoRegLRPlateau_LargeBatch_ES_div5.txt', 'w') as f:
...
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.savefig('../plots/UsingDistanceTrue_NoRegularizationLRPlateau_LargeBatch_ES_div5.pdf', bbox_inches='tight')
...
plt.title('Validation data (True distance)')
plt.xlabel('Network output')
plt.ylabel('Stars per bin')
plt.yscale('log')
plt.savefig('../plots/TrueDistance_NoRegLRPlateau_LargeBatch_ES_div5_Validation_NNout.pdf',
            bbox_inches='tight'
            )
...
plt.plot([],[], color='C1', label='Halo')
plt.legend(loc='best', frameon=False)
plt.title('Test data (True distance)')
plt.xlabel('Network output')
plt.ylabel('Stars per bin')
plt.yscale('log')
plt.savefig('../plots/TrueDistance_NoRegLRPlateau_LargeBatch_ES_div5_Test_NNout.pdf',
            bbox_inches='tight'
            )
...
plt.savefig('../plots/TrueDistance_NoRegLRPlateau_LargeBatch_ES_div5_ROC_Curve.pdf',
            bbox_inches='tight'
            )
