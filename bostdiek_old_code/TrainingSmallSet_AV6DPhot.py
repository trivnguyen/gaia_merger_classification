...
radial_velocity_error = np.load('../data/sliced_radial_velocity_error.npy')
phot_g_mean_mag = np.load('../data/sliced_phot_g_mean_mag.npy')
phot_g_mean_mag_error = np.load('../data/sliced_phot_g_mean_mag_error.npy')
phot_bp_mean_mag = np.load('../data/sliced_phot_bp_mean_mag.npy')
phot_bp_mean_mag_error = np.load('../data/sliced_phot_bp_mean_mag_error.npy')
phot_rp_mean_mag = np.load('../data/sliced_phot_rp_mean_mag.npy')
phot_rp_mean_mag_error = np.load('../data/sliced_phot_rp_mean_mag_error.npy')
...
with open('../data/MeansAndVariance.txt') as f:
    for line in f:
        ...
        if line[0] == 'pmra_true':
            ...
            normstatistics[2] = [pmra_mean, pmra_std]
        if line[0] == 'pmdec_true':
            pmdec_mean, pmdec_std = float(line[1]), np.sqrt(float(line[2]))
            normstatistics[3] = [pmdec_mean, pmdec_std]
        if line[0] == 'parallax':
            parallax_mean, parallax_std = float(line[1]), np.sqrt(float(line[2]))
            normstatistics[4] = [parallax_mean, parallax_std]
        if line[0] == 'radial_velocity':
            rv_mean, rv_std = float(line[1]), np.sqrt(float(line[2]))
            normstatistics[5] = [rv_mean, rv_std]
        if line[0] == 'phot_g_mean_mag':
            phot_g_mean_mag_mean, phot_g_mean_mag_std = float(line[1]), np.sqrt(float(line[2]))
            normstatistics[6] = [phot_g_mean_mag_mean, phot_g_mean_mag_std]
        if line[0] == 'phot_bp_mean_mag':
            phot_bp_mean_mag_mean, phot_bp_mean_mag_std = float(line[1]), np.sqrt(float(line[2]))
            normstatistics[7] = [phot_bp_mean_mag_mean, phot_bp_mean_mag_std]
        if line[0] == 'phot_rp_mean_mag':
            phot_rp_mean_mag_mean, phot_rp_mean_mag_std = float(line[1]), np.sqrt(float(line[2]))
            normstatistics[8] = [phot_rp_mean_mag_mean, phot_rp_mean_mag_std]
...
filepath = "saved_models/SmallSet_AccretedVirialized6DPhot_{epoch:02d}_{val_loss:.2f}.hdf5"
...
model.save('saved_models/SmallSet_AccretedVirialized6DPhot.h5')
...
plt.title('Measured Parallax [small errors] Accreted Virialized6DPhot')
plt.legend(loc='best', frameon=False, fontsize=14)
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.savefig('../plots/History_SmallSetAccretedVirialized6DPhot.pdf',
            bbox_inches='tight')
...
plt.title('Validation Accreted Virialized 6DPhot (Measured Parallax) [Small errors]')
plt.xlabel('Network output')
plt.ylabel('Stars per bin')
plt.yscale('log')
plt.savefig('../plots/SmallSetAccretedVirialized6DPhot_Validation_NNout.pdf',
            bbox_inches='tight'
            )
...
plt.plot([],[], color='C1', label='Halo')
plt.legend(loc='best', frameon=False)
plt.title('Test Accreted Virialized 6DPhot (Measured Parallax) [Small errors]')
plt.xlabel('Network output')
plt.ylabel('Stars per bin')
plt.yscale('log')
plt.savefig('../plots/SmallSetAccretedVirialized6DPhot_Test_NNout.pdf',
            bbox_inches='tight'
            )
...
plt.savefig('../plots/SmallSetAccretedVirialized6DPhot_ROC_Curve.pdf',
            bbox_inches='tight'
            )
