...
from merger_ml.modules import simple_fc_nn
...
def parse_cmd():
    ...
    parser.add_argument('-W', '--weight-key', required=False,
                        help='Key for weight features')
    parser.add_argument(
...
if __name__ == '__main__':
    ...
    logger.info('Weight key: {}'.format(FLAGS.weight_key))
    ...
    train_dataset = data_utils.Dataset(
        ...
        input_key=input_key, label_key=FLAGS.label_key, weight_key=FLAGS.weight_key,
    ...
    net = simple_fc_nn.SimpleFCNetwork(input_dims, l1=FLAGS.l1, l2=FLAGS.l2)
    ...
    for epoch in range(FLAGS.max_epochs):
        ...
        for i, data_b in enumerate(train_loader):
            ...
            xb = data_b[0].float().to(device)
            yb = data_b[1].float().to(device)
            if FLAGS.weight_key is not None:
                wb = data_b[2].float().to(device)
            else:
                wb = None

            yhatb = net(xb)    # forward pass
            loss = criterion(yhatb, yb, wb)    # calculate loss
